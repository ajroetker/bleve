...
// CachedDirectory is a Directory implementation that adds caching capabilities.
// Implementations can use this to cache frequently accessed files in memory
// or on local disk.
...
type CacheConfig struct {
	// MaxCacheSizeBytes is the maximum size of the cache in bytes
	MaxCacheSizeBytes int64

	// MaxCacheEntries is the maximum number of entries in the cache
	MaxCacheEntries int

	// EvictionPolicy determines how items are evicted from the cache
	// (e.g., "lru", "lfu", "fifo")
	EvictionPolicy string

	// PreloadPatterns are glob patterns for files to preload into cache
...
type CacheStats struct {
	...
	CacheEntries    int
	HitRate         float64 // calculated as Hits / (Hits + Misses)
}
